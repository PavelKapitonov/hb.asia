        <div class="special_grid">
            <div class="sp-bg-1">
                <span></span>
                <h3><?=lang('Home.text-27'); ?></h3>
                <a href="<?= base_url($locale.'/knowledge-base'); ?>" class="bt-modal"><?=lang('Buttons.readmore'); ?> <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M14 0.999999C14 0.447714 13.5523 -8.61581e-07 13 -1.11446e-06L4 -3.13672e-07C3.44772 -6.50847e-07 3 0.447715 3 0.999999C3 1.55228 3.44772 2 4 2L12 2L12 10C12 10.5523 12.4477 11 13 11C13.5523 11 14 10.5523 14 10L14 0.999999ZM1.70711 13.7071L13.7071 1.70711L12.2929 0.292893L0.292893 12.2929L1.70711 13.7071Z" />
                    </svg>
                    </a>
                <p>
            </div>
            <div class="sp-bg-2">
                <span></span>
                <h3><?=lang('Home.text-28'); ?></h3>
                <a href="<?= base_url($locale.'/aboutus'); ?>" class="bt-modal"><?=lang('Buttons.hb'); ?> <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M14 0.999999C14 0.447714 13.5523 -8.61581e-07 13 -1.11446e-06L4 -3.13672e-07C3.44772 -6.50847e-07 3 0.447715 3 0.999999C3 1.55228 3.44772 2 4 2L12 2L12 10C12 10.5523 12.4477 11 13 11C13.5523 11 14 10.5523 14 10L14 0.999999ZM1.70711 13.7071L13.7071 1.70711L12.2929 0.292893L0.292893 12.2929L1.70711 13.7071Z" />
                    </svg>
                    </a>
            </div>
            <div class="sp-bg-3">
                <span></span>
                <h3><?=lang('Home.text-30'); ?></h3>
                <a href="#modal" class="bt-modal callback"><?=lang('Buttons.popup'); ?> <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M14 0.999999C14 0.447714 13.5523 -8.61581e-07 13 -1.11446e-06L4 -3.13672e-07C3.44772 -6.50847e-07 3 0.447715 3 0.999999C3 1.55228 3.44772 2 4 2L12 2L12 10C12 10.5523 12.4477 11 13 11C13.5523 11 14 10.5523 14 10L14 0.999999ZM1.70711 13.7071L13.7071 1.70711L12.2929 0.292893L0.292893 12.2929L1.70711 13.7071Z" />
                    </svg>
                    </a>
                <p>			
            </div>
        </div>